...
export const getUsersByName = (username, req, res) => {
    ...
    const query = `SELECT * FROM user_table WHERE ${con.escapeId(userName)} LIKE ${con.escape(`%${userName}%`)}`;
    con.query(query, (err, data) => {
        if (err) {
...
export const getUserByUsername = async (username) => {
    return new Promise((resolve, reject) => {
        const query = `SELECT * FROM user_table WHERE username = ${con.escape(username)}`;
        con.query(query, (err, data) => {
            if (err) {
                reject(err);
            }
